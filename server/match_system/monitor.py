...
class MatchMonitor:
        ...
        try:
            ...
            def get_and_update_match():
                ...
                score_changed = (
                    ...
                    or match.team_b_score != team_b_score
                    or match.current_round != current_round
                )
            ...
            if changed:
                ...
                logger.info(
                    f"Match {match_id} score update: {team_a_score}-{team_b_score} "
            ...
            logger.error(f"Error updating match score: {exc}")
    ...
    async def update_player_statistics(match_id: str, player_stats: Dict) -> Dict:
        ...
        try:
            ...
            def update_stats():
                ...
                for puuid, stats in player_stats.items():
                    try:
                        ...
                        match_stat, _ = MatchStatistics.objects.get_or_create(
                            match=match,
                            player=player,
                        ...
                        total_shots = (
                            match_stat.headshots
                            + match_stat.bodyshots
                            + match_stat.legshots
                        )
                        if total_shots > 0:
                            match_stat.headshot_percentage = (
                                match_stat.headshots / total_shots
                            ) * 100
                        ...
                        logger.warning(
                            f"Player {puuid} not found for statistics update"
                        )
                        continue
            ...
            logger.info(
                f"Updated statistics for {len(player_stats)} players in match {match_id}"
            )

            return {'status': 'success'}

        except Exception as exc:  # pylint: disable=broad-except
            logger.error(f"Error updating player statistics: {exc}")
            return {'status': 'error', 'message': str(exc)}
    ...
    async def get_match_statistics(match_id: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Error getting match statistics: {exc}")
            return {'status': 'error', 'message': str(exc)}
